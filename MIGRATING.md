...
- Renamed `deleteRule` to `deleteGroupRule`; Also, the signature has changed from `delete(Boolean removeUsers)` to `deleteGroupRule()` (`removeUsers` parameter used earlier was optional and was defaulting to false).
