...
public class TerraformHelper {
    ...
    public static Object extractParameter(Map<String, Object> prerequisiteData, String parameterName, Object defaultValue, String targetClassName) {
        ...
        // 3. Add special mappings for common parameter types
        if ("appId".equals(parameterName)) {
            fieldNameVariations.add("app_id");
            fieldNameVariations.add("id");
        } else if ("userId".equals(parameterName)) {
            fieldNameVariations.add("user_id");
            fieldNameVariations.add("id");
        } else if ("groupId".equals(parameterName)) {
            fieldNameVariations.add("group_id");
            fieldNameVariations.add("id");
        } else if ("idpId".equals(parameterName)) {
            fieldNameVariations.add("idp_id");
            fieldNameVariations.add("id");
        } else if ("policyId".equals(parameterName)) {
            fieldNameVariations.add("policy_id");
            fieldNameVariations.add("id");
        }
        
        // 4. Try each field name variation in payload first
        if (prerequisiteData.containsKey("payload")) {
            Map<String, Object> payload = (Map<String, Object>) prerequisiteData.get("payload");
            if (payload != null) {
        ...
        // 5. Try each field name variation in prerequisite_object
        if (prerequisiteData.containsKey("prerequisite_object")) {
            Map<String, Object> prerequisiteObject = (Map<String, Object>) prerequisiteData.get("prerequisite_object");
            if (prerequisiteObject != null) {
                for (String fieldName : fieldNameVariations) {
                    if (prerequisiteObject.containsKey(fieldName)) {
                        return prerequisiteObject.get(fieldName);
                    }
                }
            }
        }
        
        // 6. Generic handling for object types - try targetClassName if provided,
        ...
        if (targetClassName != null && !targetClassName.isEmpty()) {
            ...
            if (objectToDeserialize != null) {
                return convertMapToSdkObject(objectToDeserialize, targetClassName);
            }
        }
    ...
    public static Object convertMapToSdkObject(Object obj, String targetClassName) {
        ...
        if (!(obj instanceof Map)) {
            return obj;  // Not a map, can't convert
        }
