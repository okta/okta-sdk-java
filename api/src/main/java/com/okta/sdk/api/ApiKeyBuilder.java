 ...
 *
 * <p>The {@code ApiKeyBuilder} is especially useful for constructing Client ApiKey instances with Okta API Key
 ...
 *         absolute file path, or it can be a URL or a classpath value by using the {@code url:} or
 *         {@code classpath:} prefixes respectively.</li>
 *     <li>{@link #setInputStream(InputStream) inputStream}: Properties discovered after reading the input
 *         stream will override any previously discovered values.</li>
 *     <li>{@link #setReader(Reader) reader}: Properties discovered after reading the Reader will override any
 *         previously discovered values.</li>
 *     <li>{@link #setProperties(Properties) properties} instance: Properties discovered in the instance
...
public interface ApiKeyBuilder {
     ...
     *
     * @param id the {@link com.okta.sdk.api.ApiKey#getBaseUrl() ApiKey id} to use when communicating with Okta.
     ...
     *
     * <pre>
     * String id = System.getenv("MY_OKTA_API_KEY_ID"); //alternative (non-default) env var name
     * String secret = System.getenv("MY_OKTA_API_KEY_SECRET"); //alternative (non-default) env var name
     * ApiKey apiKey = {@link com.okta.sdk.api.ApiKeys ApiKeys}.builder().setId(id).setSecret(secret).build();
     * Client client = {@link Clients Clients}.builder().setApiKey(apiKey).build();
     * </pre>
     *
     * @param secret the {@link com.okta.sdk.api.ApiKey#getBaseUrl() ApiKey id} to use when communicating with Okta.
     * @return the ApiKeyBuilder instance for method chaining.
     * @see #setId(String)
     * @see ClientBuilder#setApiKey(com.okta.sdk.api.ApiKey)
     */
     ...
     * following:
     * <pre>
     * String location = "/home/jsmith/.okta/apiKey.properties";
     *
     ...
     * <pre>
     * myOktaApiKeyId = foo
     * myOktaApiKeySecret = mySuperSecretValue
     * </pre>
     ...
     *          .setIdPropertyName("myOktaApiKeyId")
     *          .setSecretPropertyName("myOktaApiKeySecret")
     *          .build())
     *     .build();
     * </pre>
     *
     * @param location the file, classpath or url location of the API Key {@code .properties} file to load when
     *                 constructing the API Key to use for communicating with the Okta REST API.
     * @return the ApiKeyBuilder instance for method chaining.
     */
     ...
     *         values override any values found in the default apiKey.properties file.  The {@code OKTA_API_KEY_FILE}
     *         String can be an absolute file path, or it can be a URL or a classpath value by using the {@code url:} or
     *         {@code classpath:} prefixes respectively.</li>
     *     <li>The environment variables {@code OKTA_API_KEY_ID} and {@code OKTA_API_KEY_SECRET}.  If either of
     *         these values are present, they override any previously discovered value.</li>
     ...
     *         {@code classpath:} prefixes respectively.</li>
     *     <li>A specified {@link #setInputStream(InputStream) inputStream}: Properties discovered while
     *         reading the input stream will override any previously discovered values.</li>
     *     <li>A specified {@link #setReader(Reader) reader}: Properties discovered after reading the Reader
     *         will override any previously discovered values.</li>
     *     <li>A specified {@link #setProperties(Properties) properties} instance: Properties discovered in
