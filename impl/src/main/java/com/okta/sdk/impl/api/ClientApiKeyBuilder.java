...
public class ClientApiKeyBuilder implements ApiKeyBuilder {
    ...
    protected Properties getEnvironmentVariableFileProperties() {
        ...
        String location = System.getenv("OKTA_API_KEY_FILE");
        if (Strings.hasText(location)) {
            try {
                ...
                log.debug(
                    "Unable to load api key properties file [{}] specified by environment variable OKTA_API_KEY_FILE. This can be safely ignored as this is a fallback location - other more specific locations will be checked.",
                    location, ignored
                );
            }
        }
    ...
    protected Properties getSystemPropertyFileProperties() {
        ...
        String location = System.getProperty("okta.client.apiKey.file");
        if (Strings.hasText(location)) {
            try {
                Reader reader = createFileReader(location);
                props = toProperties(reader);
                ...
                log.debug(
                    "Unable to load api key properties file [{}] specified by system property okta.client.apiKey.file. " +
                    "This can be safely ignored as this is a fallback location - " +
                    "other more specific locations will be checked.",
                    location, ignored
                );
    ...
    protected Properties getEnvironmentVariableProperties() {
        ...
        String value = System.getenv("OKTA_API_KEY_ID");
        if (Strings.hasText(value)) {
            props.put(this.apiKeyIdPropertyName, value);
        }

        value = System.getenv("OKTA_API_KEY_SECRET");
        if (Strings.hasText(value)) {
    ...
    protected Properties getSystemProperties() {
        ...
        String value = System.getProperty("okta.client.apiKey.id");
        if (Strings.hasText(value)) {
            props.put(this.apiKeyIdPropertyName, value);
        }

        value = System.getProperty("okta.client.apiKey.secret");
        if (Strings.hasText(value)) {
            props.put(this.apiKeySecretPropertyName, value);
        }
