...
/**
 * A {@code DefaultCache} is a {@link Cache Cache} implementation that uses a backing {@link Map} instance to store and
 * retrieve cached data. <b>Thread Safety</b> This implementation is thread-safe <em>only</em> if the backing map is
 * thread-safe.
 *
 * @since 0.5.0
 */
public class DefaultCache<K, V> implements Cache<K, V> {
    ...
    /**
     * The amount of time allowed to pass since an entry was first created. An entry older than this time, <b>regardless
     * of how often it might be used</b>, will be removed from the cache as soon as possible.
     */
     ...
     * <p>
     * Finally, the {@link #setTimeToIdle(Duration) timeToIdle} and {@link #setTimeToLive(Duration) timeToLive} settings
     * are both {@code null}, indicating that cache entries will live indefinitely (except due to memory constraints as
     * managed by the {@code SoftHashMap}).
     *
     * @param name
     ...
     * <p>
     * The {@link #setTimeToIdle(Duration) timeToIdle} and {@link #setTimeToLive(Duration) timeToLive} settings are both
     * {@code null}, indicating that cache entries will live indefinitely (except due to memory constraints as managed
     * by the {@code backingMap} instance).
     *
     * @param name
     *            name to assign to this instance, expected to be unique among all other caches in the parent
     *            {@code CacheManager}.
     * @param backingMap
     *            the (ideally thread-safe) map instance to store the Cache entries.
     *
     * @see SoftHashMap
     * @see #setTimeToIdle(Duration)
     * @see #setTimeToLive(Duration)
     ...
     * <p>
     * It is expected that the {@code backingMap} implementation be thread-safe and preferrably auto-sizing based on
     * memory constraints (see {@link SoftHashMap} for such an implementation).
     *
     * @param name
     *            name to assign to this instance, expected to be unique among all other caches in the parent
     *            {@code CacheManager}.
     ...
     *            the (ideally thread-safe) map instance to store the Cache entries.
     * @param timeToIdle
     *            the amount of time cache entries may remain idle until they should be removed from the cache.
     * @param timeToLive
     *            the amount of time cache entries may exist until they should be removed from the cache.
     *
     * @throws IllegalArgumentException
     *             if either {@code timeToLive} or {@code timeToIdle} are non-null <em>and</em> represent a non-positive
     *             (zero or negative) value. This is only enforced for non-null values - {@code null} values are allowed
     *             for either argument.
     *
     * @see #setTimeToIdle(Duration)
     * @see #setTimeToLive(Duration)
    ...
    /**
     * Returns the amount of time a cache entry may exist after first being created before it will expire and no longer
     * be available. If a cache entry ever becomes older than this amount of time (regardless of how often it is
     * accessed), it will be removed from the cache as soon as possible.
     *
     * @return the amount of time a cache entry may exist after first being created before it will expire and no longer
     *         be available.
     */
    ...
    /**
     * Sets the amount of time a cache entry may exist after first being created before it will expire and no longer be
     * available. If a cache entry ever becomes older than this amount of time (regardless of how often it is accessed),
     * it will be removed from the cache as soon as possible.
     *
     * @param timeToLive
     *            the amount of time a cache entry may exist after first being created before it will expire and no
     *            longer be available.
     */
    ...
    /**
     * Returns the amount of time a cache entry may be idle - unused (not accessed) - before it will expire and no
     * longer be available. If a cache entry is not accessed at all after this amount of time, it will be removed from
     * the cache as soon as possible.
     *
     * @return the amount of time a cache entry may be idle - unused (not accessed) - before it will expire and no
     *         longer be available.
     */
    ...
    /**
     * Sets the amount of time a cache entry may be idle - unused (not accessed) - before it will expire and no longer
     * be available. If a cache entry is not accessed at all after this amount of time, it will be removed from the
     * cache as soon as possible.
     *
     * @param timeToIdle
     *            the amount of time a cache entry may be idle - unused (not accessed) - before it will expire and no
     *            longer be available.
     */
    ...
    /**
     * Returns the number of attempts to return a cache entry. Note that because {@link #remove(Object)} will return a
     * value, calls to both {@link #get(Object)} and {@link #remove(Object)} will increment this number.
     *
     ...
     * number is to {@code 1.0}, the more effectively the cache is being used. The closer this number is to {code 0.0},
     * the less effectively the cache is being used.
     *
     ...
     * to support expunging cache entries based on {@link DefaultCache#getTimeToIdle() timeToIdle} and
    ...
    public static class Entry<V> {
         ...
         * {@link #getCreationTimeMillis() creationTimeMillis} and the {@link #getLastAccessTimeMillis()
         * lastAccessTimeMills} to the current timestamp (i.e. {@link System#currentTimeMillis()}).
         *
         * @param value
         ...
         * support expunging cache entries when this value is older than the cache's {@link DefaultCache#getTimeToLive()
         * timeToLive} setting.
         *
         ...
         * support expunging cache entries when this value is older than the cache's {@link DefaultCache#getTimeToIdle()
         * timeToIdle} setting.
         *
