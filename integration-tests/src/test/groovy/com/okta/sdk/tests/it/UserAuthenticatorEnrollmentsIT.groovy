            ...
            def user = userApi.createUser(
                new com.okta.sdk.resource.model.CreateUserRequest()
                    .profile(new com.okta.sdk.resource.model.UserProfile()
                        .firstName("HeadersEnroll").lastName("Test")
                        .email("headers-enroll-${UUID.randomUUID().toString().substring(0,8)}@example.com".toString())
                        .login("headers-enroll-${UUID.randomUUID().toString().substring(0,8)}@example.com".toString())),
                true, false, null)
            userId = user.getId()

            // listAuthenticatorEnrollments with headers
            try {
                userAuthenticatorEnrollmentsApi.listAuthenticatorEnrollments(userId, headers)
