...
class ApplicationPoliciesIT extends ITSupport {
    ...
    /**
     * Test 9: Comprehensive lifecycle test
     * Tests the complete workflow: create app, assign policy, replace policy, verify throughout.
     */
    @Test
    void testComprehensivePolicyLifecycle() {
        logger.info("Testing comprehensive policy lifecycle...")

        // Create a dedicated app for this test
        String guid = UUID.randomUUID().toString()
        OpenIdConnectApplication lifecycleApp = new OpenIdConnectApplication()
        lifecycleApp.name(OpenIdConnectApplication.NameEnum.OIDC_CLIENT)
            .label(prefix + "lifecycle-test-" + guid)
            .signOnMode(ApplicationSignOnMode.OPENID_CONNECT)

        ApplicationCredentialsOAuthClient oauthClient = new ApplicationCredentialsOAuthClient()
        oauthClient.clientId("test-lifecycle-app-" + guid)
            .tokenEndpointAuthMethod(OAuthEndpointAuthenticationMethod.CLIENT_SECRET_BASIC)
            .autoKeyRotation(true)

        OAuthApplicationCredentials credentials = new OAuthApplicationCredentials()
        credentials.oauthClient(oauthClient)
        lifecycleApp.credentials(credentials)

        OpenIdConnectApplicationSettingsClient settingsClient = new OpenIdConnectApplicationSettingsClient()
        settingsClient.applicationType(OpenIdConnectApplicationType.WEB)
            .clientUri("https://lifecycle.example.com")
            .logoUri("https://lifecycle.example.com/logo.png")
            .postLogoutRedirectUris(["https://lifecycle.example.com/postlogout"])
            .responseTypes([OAuthResponseType.CODE])
            .grantTypes([GrantType.AUTHORIZATION_CODE])
            .redirectUris(["https://lifecycle.example.com/oauth/callback"])

        OpenIdConnectApplicationSettings settings = new OpenIdConnectApplicationSettings()
        settings.oauthClient(settingsClient)
        lifecycleApp.settings(settings)

        OpenIdConnectApplication createdApp = applicationApi.createApplication(lifecycleApp, true, null) as OpenIdConnectApplication
        String lifecycleAppId = createdApp.getId()
        registerForCleanup(createdApp)

        Thread.sleep(2000)

        // Step 1: Verify app initially has no policy assigned
        Application initialApp = applicationApi.getApplication(lifecycleAppId, null)
        logger.info("Step 1: App created, checking initial state...")
        // Note: accessPolicy link might be null or point to default policy initially

        // Step 2: Assign first policy
        applicationPoliciesApi.assignApplicationPolicy(lifecycleAppId, testPolicyId)
        Thread.sleep(1000)
        
        Application appWithFirstPolicy = applicationApi.getApplication(lifecycleAppId, null)
        assertThat("App should have access policy after first assignment",
            appWithFirstPolicy.getLinks().getAccessPolicy(), notNullValue())
        ...
        applicationPoliciesApi.assignApplicationPolicy(lifecycleAppId, secondPolicyId)
        Thread.sleep(1000)

        Application appWithSecondPolicy = applicationApi.getApplication(lifecycleAppId, null)
        assertThat("App should have access policy after second assignment",
            appWithSecondPolicy.getLinks().getAccessPolicy(), notNullValue())
        assertThat("Second policy should have replaced first",
            appWithSecondPolicy.getLinks().getAccessPolicy().getHref(), containsString(secondPolicyId))
        assertThat("First policy should no longer be assigned",
            appWithSecondPolicy.getLinks().getAccessPolicy().getHref(), not(containsString(testPolicyId)))
        logger.info("Step 3: Second policy replaced first successfully")

        // Step 4: Re-assign first policy
        applicationPoliciesApi.assignApplicationPolicy(lifecycleAppId, testPolicyId)
        Thread.sleep(1000)

        Application appWithFirstPolicyAgain = applicationApi.getApplication(lifecycleAppId, null)
        assertThat("App should have access policy after reassignment",
            appWithFirstPolicyAgain.getLinks().getAccessPolicy(), notNullValue())
        assertThat("First policy should be reassigned",
            appWithFirstPolicyAgain.getLinks().getAccessPolicy().getHref(), containsString(testPolicyId))
        logger.info("Step 4: First policy reassigned successfully")
